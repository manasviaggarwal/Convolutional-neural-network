...
#         return ((f1_score(testlabel, predictions, average='macro')),(f1_score(testlabel, predictions, average='micro')))	
	...
	# def new_relu_layer(self,input, name,act):
	...
	# 		return layer
    def new_relu_layer(self,act,input, name):
  ...
  #   def new_relu_layer(self,input, name,act):
    ...
    def initialization_layers_test(self,test_path,dataset):
            ...
            with tf.Session() as sess:
	            ...
	            we=graph.get_tensor_by_name("weight_convs:0")
	            print(we)
    ...
    def initialization_layers_train(self,train_path,test_path,dataset,no_of_layers,filter_sizes,act):
            global n1,n2
            x,x_image,y_true,y_true_cls=self.load_main(train_path,dataset,0)
            self.load_main(test_path,dataset,1)
            # print(len(x_image))
            ...
            layer_pool1 = self.new_pool_layer(layer_conv1, name="pool1")
            layer_relu1 = self.new_relu_layer(2,layer_pool1, name="relu1")
            ...
            for k1 in range(1,no_of_layers):
                ...
                layer_pool1 = self.new_pool_layer(layer_conv1, name=name2)
                layer_relu1 = self.new_relu_layer(2,layer_pool1, name=name1)
            ...
            layer_fc1 = self.new_fc_layer(layer_flat, num_inputs=num_features, num_outputs=128, name="fc1")
            #if act==1:
            layer_relu3 = self.new_relu_layer(act,layer_fc1, name="relu"+str(no_of_layers+1))

			# elif act==2:
   #          	layer_relu3 = self.new_relu_layer(layer_fc1, name="sigmoid"+str(no_of_layers+1))
			# elif act==3:
   #          	layer_relu3 = self.new_relu_layer(layer_fc1, name="tanh"+str(no_of_layers+1))
			# elif act==4:
   #          	layer_relu3 = self.new_relu_layer(layer_fc1, name="swish"+str(no_of_layers+1))
...
if __name__=='__main__':
    ...
    if array_of_arguments[1]=="--test-data":
        ...
        if array_of_arguments[4]=="Fashion-MNIST":
            ...
            cn.initialization_layers_test(test_path,"Fa")
        ...
        cn=CNN1()
        cn.initialization_layers_train(train_path,test_path,array_of_arguments[6],2,[4,4],1)
        t.append(train_acc)
        fmi.append(f1_mi)
        fma.append(f1_ma)
        del cn
        ...
        cn=CNN1()
        cn.initialization_layers_train(train_path,test_path,array_of_arguments[6],2,[4,4,4],2)
        t.append(train_acc)
        fmi.append(f1_mi)
        fma.append(f1_ma)
        del cn
        ...
        cn=CNN1()
        cn.initialization_layers_train(train_path,test_path,array_of_arguments[6],2,[4,4,4,4],3)
        t.append(train_acc)
        fmi.append(f1_mi)
        fma.append(f1_ma)
        del cn
        ...
        cn=CNN1()
        cn.initialization_layers_train(train_path,test_path,array_of_arguments[6],2,[4,4,4,4,4],4)
        t.append(train_acc)
        fmi.append(f1_mi)
        fma.append(f1_ma)
        del cn

        legends=['sigmoid','relu','swish','tanh']
        cwds=os.getcwd()
