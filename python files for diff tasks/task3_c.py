...
#         return ((f1_score(testlabel, predictions, average='macro')),(f1_score(testlabel, predictions, average='micro')))	
    ...
    def initialization_layers_train(self,train_path,test_path,dataset,no_of_layers,filter_sizes,v):
            ...
            # print(len(x_image))
            if dataset=="Fashion-MNIST":
                layer_conv1, weights_conv1 = self.new_conv_layer(v,input=x_image, num_input_channels=1, filter_size=8, num_filters=64, name ="conv1")
           
            elif dataset=="CIFAR-10":
                layer_conv1, weights_conv1 = self.new_conv_layer(v,input=x_image, num_input_channels=3, filter_size=8, num_filters=64, name ="conv1")
            ...
            for k1 in range(1,no_of_layers):
                namee="conv"+str(k1+1)
                layer_conv1, weights_conv1 = self.new_conv_layer(v,input=layer_relus[k1-1], num_input_channels=64, filter_size=8, num_filters=64, name= namee)
                name2="pool"+str(k1+1)
                name1="relu"+str(k1+1)
