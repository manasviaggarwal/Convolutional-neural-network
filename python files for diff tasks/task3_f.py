...
#         return ((f1_score(testlabel, predictions, average='macro')),(f1_score(testlabel, predictions, average='micro')))	
    ...
    def new_conv_layer(self,v,input, num_input_channels, filter_size, num_filters, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            if v=='g':
            	...
            	weights = tf.Variable(tf.random.uniform(shape))
    ...
    def initialization_layers_train(self,train_path,test_path,dataset,no_of_layers,filter_sizes,v):
            global n1,n2
            x,x_image,y_true,y_true_cls=self.load_main(train_path,dataset,0)
            self.load_main(test_path,dataset,1)
            # print(len(x_image))
            if dataset=="Fashion-MNIST":
                layer_conv1, weights_conv1 = self.new_conv_layer(v,input=x_image, num_input_channels=1, filter_size=10, num_filters=n1, name ="conv1")
           
            elif dataset=="CIFAR-10":
                layer_conv1, weights_conv1 = self.new_conv_layer(v,input=x_image, num_input_channels=3, filter_size=10, num_filters=n1, name ="conv1")
            ...
            for k1 in range(1,no_of_layers):
                namee="conv"+str(k1+1)
                layer_conv1, weights_conv1 = self.new_conv_layer(v,input=layer_relus[k1-1], num_input_channels=n1, filter_size=10, num_filters=n2, name= namee)
                name2="pool"+str(k1+1)
                name1="relu"+str(k1+1)
...
if __name__=='__main__':
    ...
    if array_of_arguments[1]=="--test-data":
        ...
        # for i1 in range(4):
        cn=CNN1()
        cn.initialization_layers_train(train_path,test_path,array_of_arguments[6],2,[4,4],'g')
        ...
        cn=CNN1()
        cn.initialization_layers_train(train_path,test_path,array_of_arguments[6],2,[4,4,4],'u')
        t.append(train_acc)
        fmi.append(f1_mi)
        fma.append(f1_ma)
        del cn
