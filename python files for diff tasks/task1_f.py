...
#         return ((f1_score(testlabel, predictions, average='macro')),(f1_score(testlabel, predictions, average='micro')))	
    ...
    def initialization_layers_train(self,train_path,test_path,dataset,no_of_layers,filter_sizes):
            ...
            # print(len(x_image))
            if dataset=="Fashion-MNIST":
                layer_conv1, weights_conv1 = self.new_conv_layer(input=x_image, num_input_channels=1, filter_size=filter_sizes[0], num_filters=12, name ="conv1")
           
            elif dataset=="CIFAR-10":
                layer_conv1, weights_conv1 = self.new_conv_layer(input=x_image, num_input_channels=3, filter_size=filter_sizes[0], num_filters=12, name ="conv1")
            ...
            for k1 in range(1,no_of_layers):
                namee="conv"+str(k1+1)
                layer_conv1, weights_conv1 = self.new_conv_layer(input=layer_relus[k1-1], num_input_channels=n2, filter_size=filter_sizes[k1], num_filters=20, name= namee)
                name2="pool"+str(k1+1)
                name1="relu"+str(k1+1)
...
if __name__=='__main__':
    ...
    if array_of_arguments[1]=="--test-data":
        ...
        del cn
        cn=CNN1()
        cn.initialization_layers_train(train_path,test_path,array_of_arguments[6],3,[4,4,4])
        t.append(train_acc)
        fmi.append(f1_mi)
        fma.append(f1_ma)
        del cn
        cwds=os.getcwd()
        cn=CNN1()
        cn.initialization_layers_train(train_path,test_path,array_of_arguments[6],4,[4,4,4,4])
        t.append(train_acc)
        fmi.append(f1_mi)
        fma.append(f1_ma)
        del cn
        cn=CNN1()
        cn.initialization_layers_train(train_path,test_path,array_of_arguments[6],5,[4,4,4,4,4])
        t.append(train_acc)
        fmi.append(f1_mi)
        fma.append(f1_ma)
        del cn
